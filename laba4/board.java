package labs.lab4;
import labs.lab4.src.Bishop;
import labs.lab4.src.Figure;
import labs.lab4.src.King;
import labs.lab4.src.Knight;
import labs.lab4.src.Pawn;
import labs.lab4.src.Queen;
import labs.lab4.src.Rook;
...
{  private Figure  fields[][] = new Figure[8][8];
    ...
    public void move_figure(int row1, int col1, int row2, int col2) throws Error {
        Figure figure = getFigureInNewPosition(row1, col1);
    ...
    private Figure getFigureInNewPosition(int row, int col) throws Error {
        try {
    ...
    private void movePawn(int row1, int col1, int row2, int col2, Figure figure) throws Error {
        Figure nextField = getFigureInNewPosition(row2, col2);
        if (nextField != null && figure.canAttack(row1, col1, row2, col2, nextField)) {
            attackFigure(row1, col1, row2, col2, figure);
            return;
    ...
    private void moveOtherFigure(int row1, int col1, int row2, int col2, Figure figure) throws Error {
        if (figure.canMove(row1, col1, row2, col2, this.fields)) {
            ...
            if (figure.getClass().getSimpleName().equals("King") && this.Check(row2, col2, this.getColorGaming())) {
    ...
    private void CheckMate(int row, int col){
        ...
        if (this.Check(oppositeKingPos[0], oppositeKingPos[1], oppositeKingColor)) {
            System.out.println("Шах вражескому королю!");
            if (this.Mate(row, col, oppositeKingPos[0], oppositeKingPos[1], oppositeKingColor)) {
                System.out.println("Вражескому королю поставлен мат! Игра закончена");
            }
        }
    }

    public boolean Mate(int row, int col, int kingRow, int kingCol, char kingColor) {
        if (!canKingLeave(kingRow, kingCol, kingColor)) {
            return false;
        }

        if (!canKingProtect(row, col, kingColor)) {
            return false;
        }

        return true;
    }
    private boolean canKingLeave(int kingRow, int kingCol, char kingColor) {
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (kingRow + i >= 8 || kingRow + i < 0 || kingCol + j >= 8 || kingCol + j < 0) continue;
                if (i == 0 && j == 0) continue;
                if (this.fields[kingRow + i][kingCol + j] == null && !(this.Check(kingRow + i, kingCol + j, kingColor))) {
                    return true;
                }
            }
        }
    ...
    private boolean canKingProtect(int row, int col, char kingColor) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                if (figure.getClass().getSimpleName().equals("King") && this.Check(row, col, kingColor)) {
                    continue;
                }
    ...
    public boolean Check(int kingRow, int kingCol, char kingColor) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                Figure figure = this.fields[i][j];
                if (figure == null || figure.getColor() == kingColor) {
    ...
    public void attackFigure(int row1, int col1, int row2, int col2, Figure figure) {
