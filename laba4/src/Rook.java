...
public class Rook extends Figure{
    ...
    public boolean canMove(int row, int col, int row1, int col1, Figure[][] fields) {
        if (row == row1 || col == col1){
            ...
            if (Math.abs(row - row1) != 0) {
                ...
                if (!CheckingFields(row, row1, direction, col, fields)) return false;
            }
            if (Math.abs(col - col1) != 0) {
                direction = col < col1 ? 1 : -1;
                if (!CheckingFields(col, col1, direction, row, fields)) return false;
            }
            return true;
        }
        return false;
    ...
    private boolean CheckingFields(int start, int end, int step, int index, Figure[][] fields) {
